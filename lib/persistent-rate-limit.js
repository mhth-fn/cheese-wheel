...
function createPersistentRateLimiter({
  ...
  `);
  const getBucketStatement = db.prepare(`
  ...
  function consume(scope, key, max, windowMs) {
    ...
    const alreadyLimited = !row;
    if (!row) {
      row = getBucketStatement.get(scope, keyHash);
      if (!row) {
        throw new Error('Rate-limit bucket disappeared during consume');
      }
    }
