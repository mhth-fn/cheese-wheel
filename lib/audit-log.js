...
function routeToAction(method, requestPath) {
  ...
  const exact = {
    ...
    'POST /api/admin/invitations': 'invitation.created',
    'POST /api/admin/portable-backup': 'backup.downloaded',
  ...
  if (key === 'PATCH /api/users/:id/name') return 'user.name_changed';
  if (/^POST \/api\/invitations\/[^/]+$/u.test(key)) return 'invitation.accepted';
